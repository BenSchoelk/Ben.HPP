...
class _ContestScreen extends State<ContestScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    context.read<ContestCubit>().getContest(context.read<UserDetailsCubit>().getUserId());
  }
            ...
            appBar: AppBar(
                ...
                title: Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(color: Theme.of(context).primaryColor, fontWeight: FontWeight.bold, fontSize: 22.0),
                  ),
                ),
                bottom: TabBar(
                    ...
                    indicatorWeight: 5,
                    tabs: [
                      Tab(text: AppLocalization.of(context)!.getTranslatedValues("pastLbl")),
                      Tab(text: AppLocalization.of(context)!.getTranslatedValues("liveLbl")),
                      Tab(text: AppLocalization.of(context)!.getTranslatedValues("upcomingLbl")),
                    ])),
            body: Stack(
              children: [
                ...
                BlocConsumer<ContestCubit, ContestState>(
                    ...
                    builder: (context, state) {
                      ...
                      return TabBarView(children: [past(contestList.past), live(contestList.live), future(contestList.upcoming)]);
                    })
              ],
            ),
          );
    ...
    return data.errorMessage.isNotEmpty
        ...
        : ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return Container(
                  height: data.contestDetails[index].showDescription == false ? MediaQuery.of(context).size.height * .3 : MediaQuery.of(context).size.height * .4,
    ...
    return Column(
      children: [
        ...
        Expanded(
          flex: 2,
          child: Container(
            color: Theme.of(context).backgroundColor, //height: MediaQuery.of(context).size.height*.05,
            child: Row(
              ...
              children: [
                ...
                GestureDetector(
                    onTap: () {
                      setState(() {
                        data.contestDetails[index].showDescription = !data.contestDetails[index].showDescription!;
                      });
                    },
                    child: Icon(
                      data.contestDetails[index].showDescription! ? Icons.keyboard_arrow_up_sharp : Icons.keyboard_arrow_down_sharp,
                      color: Theme.of(context).primaryColor,
                      size: 40,
                    )),
              ],
        ...
        Expanded(
          ...
          child: Container(
            padding: EdgeInsets.only(left: 10),
            decoration: BoxDecoration(color: Theme.of(context).backgroundColor, borderRadius: BorderRadius.only(bottomLeft: Radius.circular(20), bottomRight: Radius.circular(20))),
            child: Row(
              children: [
                Expanded(
                  flex: 1,
                ...
                Expanded(
                  ...
                  child: Column(
                    children: [
                      Text(
                        AppLocalization.of(context)!.getTranslatedValues("endsOnLbl")!,
                        style: TextStyle(color: Theme.of(context).primaryColor.withOpacity(0.6), fontWeight: FontWeight.bold),
                      ),
                      Text(
                ...
                Expanded(
                  ...
                  child: Column(
                    children: [
                      Text(
                        AppLocalization.of(context)!.getTranslatedValues("playersLbl")!,
                        style: TextStyle(color: Theme.of(context).primaryColor.withOpacity(0.6), fontWeight: FontWeight.bold),
                      ),
                      Text(
                    ...
                    type == 0
                        ? TextButton(
                            style: TextButton.styleFrom(
                              ...
                              minimumSize: Size(MediaQuery.of(context).size.width * .1, MediaQuery.of(context).size.height * .05),
                              shape: new RoundedRectangleBorder(
                                borderRadius: new BorderRadius.circular(10.0),
                              ),
                            ),
                            onPressed: () {
                              Navigator.of(context).pushNamed(Routes.contestLeaderboard, arguments: {"contestId": data.contestDetails[index].id});
                            },
                            child: Text(
                            ...
                            ? Padding(
                                ...
                                child: ElevatedButton(
                                  style: ElevatedButton.styleFrom(
                                    ...
                                    minimumSize: Size(MediaQuery.of(context).size.width * .2, MediaQuery.of(context).size.height * .05),
                                    shape: new RoundedRectangleBorder(
                                      borderRadius: new BorderRadius.circular(10.0),
                                    ),
                                  ),
                                  onPressed: () {
                                    if (int.parse(context.read<UserDetailsCubit>().getCoins()!) >= int.parse(data.contestDetails[index].entry!)) {
                                      context.read<UserDetailsCubit>().updateCoins(addCoin: false, coins: int.parse(data.contestDetails[index].entry!));
                                      Navigator.of(context).pushReplacementNamed(Routes.quiz, arguments: {"numberOfPlayer": 1, "quizType": QuizTypes.contest, "contestId": data.contestDetails[index].id, "quizName": "Contest"});
                                    } else {
                                      UiUtils.setSnackbar(AppLocalization.of(context)!.getTranslatedValues("noCoinsMsg")!, context, false);
                                    }
                                  },
                                  child: Text(
