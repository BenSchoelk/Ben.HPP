...
class ExamCubit extends Cubit<ExamState> {
  ...
  void startExam({required Exam exam, required String userId}) async {
    ...
    try {
      ...
      emit(ExamFetchSuccess(exam: exam, questions: arrangeQuestions(questions)));
    } catch (e) {
  ...
  List<Question> arrangeQuestions(List<Question> questions) {
    ...
    List<String> marks = questions.map((question) => question.marks!).toSet().toList();
    //sort marks
    marks.sort((first, second) => first.compareTo(second));

    //arrange questions from low to high mrak
    marks.forEach((questionMark) {
      arrangedQuestions.addAll(questions.where((element) => element.marks == questionMark).toList());
    });

    print(arrangedQuestions);
  ...
  int getQuetionIndexById(String questionId) {
    if (state is ExamFetchSuccess) {
      return (state as ExamFetchSuccess).questions.indexWhere((element) => element.id == questionId);
    }
    return 0;
  }
  ...
  void updateQuestionWithAnswer(String questionId, String submittedAnswerId) {
    if (state is ExamFetchSuccess) {
      ...
      emit(ExamFetchSuccess(exam: (state as ExamFetchSuccess).exam, questions: updatedQuestions));
    }
  }
  ...
  void submitResult({required String userId, required String totalDuration}) {
    if (state is ExamFetchSuccess) {
      ...
      getUniqueQuestionMark().forEach((mark) {
        ...
        int correctAnswers = questions.where((element) => element.submittedAnswerId == element.correctAnswerOptionId).toList().length;
        Statistics statistics = Statistics(mark: mark, correctAnswer: correctAnswers.toString(), incorrect: (questions.length - correctAnswers).toString());
  ...
  List<String> getUniqueQuestionMark() {
    if (state is ExamFetchSuccess) {
      return (state as ExamFetchSuccess).questions.map((question) => question.marks!).toSet().toList();
    }
    return [];
  }
