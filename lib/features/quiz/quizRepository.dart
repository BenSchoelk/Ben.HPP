...
class QuizRepository {
  ...
  Future<List<Subcategory>> getSubCategory(String category) async {
    try {
      ...
      List result = await _quizRemoteDataSource.getSubCategory(category);
      subCategoryList = result.map((subCategory) => Subcategory.fromJson(Map.from(subCategory))).toList();
  ...
  Future<int> getUnlockedLevel(String? userId, String? category, String? subCategory) async {
    try {
      final result = await _quizRemoteDataSource.getUnlockedLevel(userId, category, subCategory);
  ...
  Future<void> updateLevel({String? userId, String? category, String? subCategory, String? level}) async {
    try {
      print("Category Id : $category And Sub-category Id : $subCategory");
      await _quizRemoteDataSource.updateLevel(category: category, level: level, subCategory: subCategory, userId: userId);
    } catch (e) {
      throw QuizException(errorMessageCode: e.toString());
    }
  }
  ...
  Future<List<Question>> getQuestions(QuizTypes? quizType,
    ...
    try {
      ...
      if (quizType == QuizTypes.dailyQuiz) {
        result = await _quizRemoteDataSource.getQuestionsForDailyQuiz(languageId: languageId, userId: userId);
        ...
        if (level! == "0") {
          ...
          result = await _quizRemoteDataSource.getQuestionByCategoryOrSubcategory(type: type, id: id);
        } else {
          result = await _quizRemoteDataSource.getQuestionsForQuizZone(languageId: languageId!, categoryId: categoryId!, subcategoryId: subcategoryId!, level: level);
        }
  ...
  Future<void> setContestLeaderboard({String? userId, String? contestId, int? questionAttended, int? correctAns, int? score}) async {
    try {
      await _quizRemoteDataSource.setContestLeaderboard(userId: userId, contestId: contestId, questionAttended: questionAttended, correctAns: correctAns, score: score);
    } catch (e) {
      throw QuizException(errorMessageCode: e.toString());
    }
  }
  ...
  Future getContestLeaderboard({String? userId, String? contestId}) async {
    try {
      ...
      List result = await _quizRemoteDataSource.getContestLeaderboard(contestId, userId /*as Future<List<dynamic>>*/);
      contestLeaderboardList = result.map((category) => ContestLeaderboard.fromJson(Map.from(category))).toList();
